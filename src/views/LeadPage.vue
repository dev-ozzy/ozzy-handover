...
const normalizeAndValidateNomorLeads = (input) => {
  const raw = (input || "").trim();

  // buang karakter non digit, tapi pertahankan info apakah dia pakai +
  const hadPlus = raw.startsWith("+");
  const digitsOnly = raw.replace(/\D/g, ""); // ambil digit saja

  // CASE 1: Indonesia (08... / 62... / +62...)
  // Pakai raw yang dibersihin ringan supaya 0 tidak hilang
  const rawClean = raw.replace(/[\s\-\(\)]/g, "");

  if (/^(?:\+?62|0)8\d{8,12}$/.test(rawClean)) {
    let n = rawClean;
    // normalisasi jadi 08...
  ...
  // CASE 2: Internasional E.164 (+ maksimal 15 digit angka)
  // Harus ada + untuk non-ID
  const intl = hadPlus ? `+${digitsOnly}` : rawClean; // kalau user memang pakai +, bentukin ulang
  if (/^\+\d{6,15}$/.test(intl)) {
...
const validateOnlineForm = () => {
  ...
  if (!modalData.nomor_leads) {
    ...
    const res = normalizeAndValidateNomorLeads(modalData.nomor_leads);

    if (!res.ok) {
      errors.nomor_leads =
        "Format No Leads tidak valid (contoh: 08123456789 / +6281234567890 / +22670123456)";
      isValid = false;
    } else {
      modalData.nomor_leads = res.value;
    }
  }
